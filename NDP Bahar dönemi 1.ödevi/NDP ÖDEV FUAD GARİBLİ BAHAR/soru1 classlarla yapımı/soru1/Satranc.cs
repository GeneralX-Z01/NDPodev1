...
namespace soru1
{
    class Satranc
    {
        public void kale_yaz()//kalelelerin yerleştirileceği tahtayı oluşturan fonksiyon
        {
            for (int i = 1; i < 73; i++)
            {
                for (int j = 0; j < 8; j++)
        ...
        public void yazdir(int x, int y, string c)
        {
            Console.SetCursorPosition(x, y);
            Console.Write(c);
        }
        public void kale_olustur(int[] kale_konum)//kale koordinatlarının oluşturulduğu ve kontrol edildiği fonksiyon 
        {
            int kontrol = 0;
        ...
        public void kale_yerlestir(int[] kale_x, int[] kale_y)//konumlara göre kalelerin yerleştirildiği fonksiyon
        {
            Satranc program = new Satranc();
